...
const Hero = ({ onOpenTriage }) => {
  ...
  return (
    /* Natural Hero Enclave: Removes artificial height squishing so content flows gracefully beneath the floating navbar at 100% zoom! */
    <section ref={scrollRef} className="relative bg-[#07130F] overflow-hidden">
      ...
      <div className="max-w-[1600px] w-full mx-auto relative overflow-hidden">
        ...
        <div className="absolute top-0 right-0 bottom-0 w-[60%] sm:w-[58%] md:w-[56%] lg:w-[55%] z-0 hidden md:block overflow-hidden bg-[#07130F]">
          ...
          <img
            ...
            className="w-full h-full object-cover object-[45%_top] lg:object-[42%_top] scale-100 transform-gpu transition-transform duration-700"
          ...
          {/* 1. Base contour spine mask spanning only the leftmost 25% boundary */}
          ...
          {/* 3. Middle Bending Contour (+14deg) */}
        ...
        {/* ── Main Content Container: pt-44/pt-48 guarantees vast clearance below navbar at 100% zoom! ── */}
        <div className="relative z-10 max-w-[1360px] w-full mx-auto px-5 sm:px-8 md:px-12 pt-36 sm:pt-40 md:pt-44 lg:pt-48 pb-14 sm:pb-16 md:pb-20 my-auto">
          
          {/* ── MOBILE VIEW ONLY (Hidden on Desktop & Laptop screens) ── */}
          <motion.div
          ...
          {/* ── LEFT COLUMN TYPOGRAPHY (Scaled to replicate the luxurious 67%-zoom proportions at 100% zoom!) ── */}
          <div className="w-full max-w-[560px] mx-auto md:mx-0">
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5 }}
              className="inline-flex items-center gap-2 sm:gap-2.5 px-3.5 py-1.5 sm:px-4 sm:py-2 rounded-full bg-white/10 backdrop-blur-md text-white mb-5 border border-white/20 w-fit shadow-sm"
            >
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <span className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-emerald-300 flex items-center gap-1.5 font-mono-tech">
                <MapPin className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-[#D2A13E]" />
                <span>Home Visits • Bommasandra, Bengaluru</span>
              </span>
            </motion.div>

            {/* ── Slogan Headline: Scaled for crisp 100% zoom elegance ── */}
            <motion.h1 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.6, delay: 0.1 }}
              className="text-3xl sm:text-4xl md:text-5xl lg:text-[52px] font-black tracking-tight text-white leading-[1.12] mb-3.5 font-sans"
            >
              Bend,{' '}
            ...
            <motion.p 
              ...
              className="text-xs sm:text-sm md:text-[15px] text-white/90 font-normal mb-6 leading-relaxed font-sans max-w-[500px]"
            >
            ...
            {/* ── Interactive Tab Briefing (Sleek Glassmorphism Card) ── */}
            <motion.div 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, delay: 0.3 }}
              className="w-full max-w-[520px] bg-white/10 backdrop-blur-xl rounded-2xl p-4 sm:p-5 border border-white/15 shadow-2xl mb-7"
            >
              <div className="flex items-center gap-1.5 sm:gap-2 pb-2.5 sm:pb-3 border-b border-white/10 overflow-x-auto scrollbar-hide">
                {tabs.map((tab) => (
                  <button
                    key={tab.id}
                    onClick={() => setActiveTab(tab.id)}
                    className={`px-3 py-1.5 rounded-xl text-[11px] sm:text-xs font-bold transition-all cursor-pointer whitespace-nowrap ${
                      activeTab === tab.id
              ...
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="pt-3"
                >
                  <span className="inline-block px-2 py-0.5 rounded-md bg-emerald-400/20 text-emerald-300 font-mono-tech text-[10px] font-bold uppercase mb-1.5">
                    {activeTabData?.badge}
                  </span>
                  <h3 className="text-sm sm:text-base md:text-[17px] font-extrabold text-white mb-1 font-sans">
                    {activeTabData?.title}
                  </h3>
                  <p className="text-[11px] sm:text-xs text-white/85 leading-relaxed font-medium">
            ...
            <motion.div 
              ...
              className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3 w-full sm:w-auto mb-8"
            >
              <button
                onClick={onOpenTriage}
                className="h-12 px-6 rounded-2xl bg-white text-[#0A1C17] font-black text-xs uppercase tracking-wider shadow-lg hover:bg-[#D2A13E] hover:text-white hover:shadow-2xl hover:-translate-y-0.5 active:translate-y-0 transition-all cursor-pointer flex items-center justify-center gap-2.5 group"
              >
                <HeartPulse className="w-4 h-4 text-[#C2593B] group-hover:text-white group-hover:scale-110 transition-all shrink-0" />
              ...
              <button
                ...
                className="h-12 px-5 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 text-white font-bold text-xs hover:bg-white/20 transition-all cursor-pointer flex items-center justify-center gap-2"
              >
            ...
            <motion.div 
              ...
              className="flex flex-wrap items-center gap-3.5 text-[11px] sm:text-xs text-white/75 font-mono-tech font-bold"
            >
              <span className="flex items-center gap-1.5">
                <Award className="w-3.5 h-3.5 text-[#D2A13E]" />
                DPT Accredited
              </span>
              ...
              <span className="flex items-center gap-1">
                {[...Array(5)].map((_, i) => <Star key={i} className="w-2.5 h-2.5 fill-[#D2A13E] text-[#D2A13E]" />)}
      ...
      {/* ── Metrics Strip (outside the hero portrait canvas) ── */}
      <div className="relative z-10 bg-[#FAF8F5]">
        <div className="max-w-[1360px] mx-auto px-5 md:px-12 py-7 sm:py-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            <div className="p-4 rounded-2xl bg-white border border-[#0A1C17]/10 shadow-sm">
              <p className="text-2xl sm:text-3xl font-black text-[#0A1C17] mb-1 font-sans">
            ...
            <div className="p-4 rounded-2xl bg-white border border-[#0A1C17]/10 shadow-sm">
              <p className="text-2xl sm:text-3xl font-black text-[#C2593B] mb-1 font-sans">
            ...
            <div className="p-4 rounded-2xl bg-white border border-[#0A1C17]/10 shadow-sm">
              <p className="text-2xl sm:text-3xl font-black text-[#0A1C17] mb-1 font-sans">4–6 Wks</p>
              <p className="text-[10px] md:text-xs font-mono-tech text-[#0A1C17]/70 font-semibold uppercase tracking-wider">Avg Discharge</p>
            </div>
            <div className="p-4 rounded-2xl bg-white border border-[#0A1C17]/10 shadow-sm">
              <p className="text-2xl sm:text-3xl font-black text-emerald-800 mb-1 font-sans">
